 ...
 */
function DashboardContent({ products }: { products: CatalogProduct[] }) {
  const quickStats = [{ label: 'Produtos no portfólio', value: String(products.length), icon: Package }, ...quickStatsBase];
      ...
      <div className="mx-auto max-w-[1440px] px-4 pb-20 sm:px-6 lg:px-10">
        ...
        <section id="mais-vendidos" className="scroll-mt-24 py-8">
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {products.filter((product) => product.bestSeller || product.hot).slice(0, 3).map((product) => <ProductCard key={product.slug} product={product} />)}
          </div>
        </section>
        ...
        <section id="portfolio" className="scroll-mt-24 py-12">
          ...
          <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            ...
            {filteredProducts.length === 0 && <div className="col-span-full rounded-2xl border border-dashed border-black/15 p-12 text-center text-sm text-[#718096]">{products.length === 0 ? 'Novas soluções chegando em breve. Peça um orçamento para um projeto sob medida.' : 'Nenhuma solução encontrada. Tente outra busca ou categoria.'}</div>}
          </div>
        </section>
...
export default function DashboardView({ products }: { products: CatalogProduct[] }) {
